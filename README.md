...
- **多模型支持**：支持 YOLOv8/YOLO11 水平框 (HBB) 和 旋转框 (OBB) 模型，适配航拍/卫星视角。
- **高性能批量处理**：
  - 支持 **批量导入** 影像文件夹。
  - **硬件加速**：自动调用 Apple Silicon (MPS) 或 NVIDIA GPU (CUDA) 进行加速。
  - **批量推理**：优化的 Batch Inference 策略，大幅提升检测速度。
  - **任务控制**：支持随时 **🛑 终止** 正在进行的任务。
- **大图切片检测**：内置滑动窗口 (Sliding Window) 算法，支持超大分辨率 GeoTIFF 影像的自动切片与结果合并。
- **广泛的格式支持**：不仅支持 GeoTIFF，还支持普通图片格式 (JPG, PNG) 的导入与分析。
- **交互式可视化**：
  - 支持缩放、平移的地图视图。
  - **影像导航**：支持上一张/下一张快捷切换。
  - **热力图模式**：直观展示目标分布密度。
